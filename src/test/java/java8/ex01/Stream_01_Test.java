...
public class Stream_01_Test {
	...
	public void test_stream_filter() throws Exception {
		...
		//List<Pizza> result = pizzas.stream().filter(piz -> piz.getPrice()>=1300).toList();
	...
	public void test_stream_filter_collect_counting() throws Exception {
		...
		//long result = pizzas.stream().filter(piz -> piz.getPrice()>=1300).count();
		long result = pizzas.stream().filter(piz -> piz.getPrice()>=1300).collect(Collectors.counting());
	...
	public void test_stream_anyMatch() throws Exception {
		...
		Boolean result1 = pizzas.stream().anyMatch(piz -> piz.getPrice()>=1300);

		// TODO valider si au moins une pizza à un prix >= 2000
		Boolean result2 = pizzas.stream().anyMatch(piz -> piz.getPrice()>=2000);;
	...
	public void test_stream_allMatch() throws Exception {
		...
		Boolean result1 = pizzas.stream().allMatch(piz -> piz.getPrice()>=1300);

		// TODO valider que toutes les pizzas ont un prix >= 900
		Boolean result2 = pizzas.stream().allMatch(piz -> piz.getPrice()>=900);
	...
	public void test_stream_noneMatch() throws Exception {
		...
		Boolean result1 = pizzas.stream().noneMatch(piz -> piz.getPrice()>=2000);
	...
	public void test_stream_findFirst() throws Exception {
		...
		Optional<Order> result = orders.stream().filter(or -> or.getCustomer().getFirstname().equals("Sophie")).findFirst();
	...
	public void test_stream_filter_adv() throws Exception {
		...
		List<Order> result = orders.stream()
									.filter(o -> o.getPizzas().stream()
																.anyMatch(piz -> piz.getName().equals("Pépéroni")))
